...
export function buildPosterPrompt({ album, artist, year, tags, review, accentColor, style = 'glass' }) {
  ...
  parts.push(`Music album poster: "${album}" by ${artist}${year ? ` (${year})` : ''}. ${styleDesc}.`)
  ...
  if (accentColor) {
    parts.push(`Color theme: #${accentColor.replace('#', '')}.`)
  }

  if (review) {
    parts.push(`Review quote: "${review}"`)
  }

  parts.push('Portrait 9:16, social media share card.')
