...
export async function fetchAIPoster({ album, artist, year, tags, review, accentColor, style }) {
  const res = await fetch('/api/generate-ai-poster', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ album, artist, year, tags, review, accentColor, style }),
  })

  const json = await res.json()
  if (!res.ok || !json.success) {
    throw new Error(json?.error || 'AI 海报生成失败')
  }
...
export async function downloadAIPoster({ album, artist, year, tags, review, accentColor, style }) {
  const blob = await fetchAIPoster({ album, artist, year, tags, review, accentColor, style })
