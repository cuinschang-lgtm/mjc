...
export async function fetchAIPoster({ album, artist, year, tags, review, accentColor, style, coverImageUrl }) {
  const res = await fetch('/api/generate-ai-poster', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ album, artist, year, tags, review, accentColor, style, coverImageUrl }),
  })
  ...
  if (!res.ok || !json.success) {
    const msg = json?.hint
      ? `${json.error}\n\n诊断: ${json.hint}`
      : (json?.error || 'AI 海报生成失败')
    throw new Error(msg)
  }

  const blob = base64ToBlob(json.imageBase64, 'image/png')
  return blob
}

export async function downloadAIPoster({ album, artist, year, tags, review, accentColor, style, coverImageUrl }) {
  const blob = await fetchAIPoster({ album, artist, year, tags, review, accentColor, style, coverImageUrl })
  const filename = buildPosterFilename({ album, artist })
