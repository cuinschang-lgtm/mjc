const BASE_URL = process.env.POSTER_AI_BASE_URL || 'https://api.302.ai'
const API_KEY = process.env.POSTER_AI_API_KEY || ''
...
async function callKolors(prompt) {
  ...
  const url = `${BASE_URL}/siliconflow/v1/images/generations`
  const res = await fetch(url, {
    method: 'POST',
